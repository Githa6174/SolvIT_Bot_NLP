...
# --- 2. MUAT MODEL & DATASET ---
print("Memuat Otak AI dari file .pkl dan .csv...")
...
# Siapkan Sastrawi untuk memproses chat dari user (wajib sama dengan saat training)
...
PENTING: Anda HANYA boleh menjawab pertanyaan seputar masalah IT (komputer, jaringan, software, hardware, dll) atau sapaan ramah. Jika pertanyaan pengguna SAMA SEKALI BUKAN TENTANG IT (misal: politik, resep masakan, dll), tolak dengan sopan dan ingatkan bahwa Anda adalah Asisten IT Helpdesk. Format teks Anda dengan HTML tags dasar Telegram (<b> untuk tebal, <i> untuk miring, <code> untuk command/perintah)."""
    try:
...
def cari_solusi(chat_user):
    ...
    # Memprediksi Intent menggunakan MultinomialNB
    ...
    # Filter pencarian kemiripan HANYA pada data dengan intent yang sama
    ...
    # Jika sistem kurang yakin (akurasi di bawah 15%)
    if skor_tertinggi < 0.15:
        # Gunakan Gemini murni sebagai fallback tanpa konteks database
        return fallback_gemini(chat_user)
    
    # Jika yakin, ambil konteks dari database
    ...
    # Gunakan Gemini (RAG) untuk merangkai jawaban yang luwes dan ramah
    jawaban = generate_jawaban_gemini(chat_user, intent_formatted, pertanyaan_terkait, solusi)
    return jawaban

# --- 4. ROUTING PESAN TELEGRAM ---
